...
local notifconf = require("notifications")
...
-- What happends when you use different clicks on the taksk list (at the top)
...
local testtextbox = wibox.widget.textbox("Hello world")

-- Create the battery widget
local battery_bar = wibox.widget {
    color = beautiful.color9,
    background_color = beautiful.background,
    value = 0,
    border_color = beautiful.color9,
    border_width = 4,
...
local function update_widget_battery()
    awful.spawn.easy_async("cat /sys/class/power_supply/BAT0/capacity", function(output)
        battery_bar.value = tonumber(output)
    end)
end

-- Create the timer to update the battery value
local battery_timer = gears.timer {
    timeout = 90,
    autostart = true,
    call_now = true,
    callback = update_widget_battery,
    single_shot = false
}
...
local volume_bar = wibox.widget {
    ...
    background_color = beautiful.background,
    value = 0,
    border_color = beautiful.color2,
    border_width = 4,
    bar_border_color = beautiful.background,
    bar_border_width = 0,
    shape = gears.shape.rounded_bar,
    bar_shape = gears.shape.rounded_bar,
    max_value = 100,
...
local function update_widget_volume()
    awful.spawn.easy_async("pamixer --get-volume-human", function(output)
        if output == "muted" then
            volume_bar.value = 0
        else
            volume_bar.value = tonumber(output:match("%d+"))
        end
    end)
end

-- Connect the signal for the volume
awesome.connect_signal("volume_changed", function()
    update_widget_volume()
end)
...
local function update_widget_brightness()
    awful.spawn.easy_async("xbacklight -get", function(output)
        brightness_bar.value = tonumber(output)
    end)
end

-- Connect the signal for the brightness
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create a textbox for the screen index
    s.indexbox = wibox.widget.textbox(s.index)
...
globalkeys = gears.table.join(
    ...
    -- Sound control (pulseaudio)
    awful.key({}, "F9",
        function ()
            awful.spawn.easy_async_with_shell( "pamixer -u; pamixer -d 5", function()
    ...
    awful.key({}, "F10",
        function ()
            awful.spawn.easy_async_with_shell( "pamier -u; pamixer -i 5", function()
                awesome.emit_signal("volume_changed")
            end)
        end,
    ...
    awful.key({}, "F8",
        ...
        { description = "Mute default audio sink", group = "awesome" }
    ),

    -- Brightness control (xbacklight)
    awful.key({}, "F11",
        function ()
            awful.spawn.easy_async( "xbacklight -dec 8", function()
    ...
    awful.key({}, "F12",
        function ()
            awful.spawn.easy_async( "xbacklight -inc 8", function()
                awesome.emit_signal("brightness_changed")
            end)
        end,
